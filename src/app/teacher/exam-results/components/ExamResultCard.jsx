    ...
    <div className='flex flex-col h-full'>
          ...
          <div className='px-5 py-4 space-y-3'>
            ...
            {ujian.avgScore !== undefined && ujian.avgScore !== null && (
              <div className='flex items-center justify-between pt-2 border-t border-gray-100'>
                ...
                <span className={`text-lg font-bold ${avg >= 75 ? 'text-green-600' : avg >= 50 ? 'text-yellow-600' : 'text-red-500'}`}>
                  {avg.toFixed(1)}
                </span>
              </div>
            )}

            {isArchived && ujian.submittedAt && (
              <div className='flex items-center justify-between pt-2 border-t border-gray-100'>
                <span className='text-sm text-gray-500'>Diarsipkan</span>
                <span className='text-xs text-gray-600'>{new Date(ujian.submittedAt).toLocaleDateString('id-ID')}</span>
              </div>
            )}
          </div>
        </Card>
      ...
      {!isArchived && onSubmit && (
        <Button
          variant='outline'
          size='sm'
          className='mt-2 w-full text-xs text-gray-500 border-dashed hover:border-orange-400 hover:text-orange-600 hover:bg-orange-50'
          onClick={(e) => { e.preventDefault(); onSubmit(ujian.id, ujian.examName); }}
        >
