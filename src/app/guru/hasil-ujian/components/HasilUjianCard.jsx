...
export default function HasilUjianCard({ ujian }) {
  return (
    <Link href={`/guru/hasil-ujian/list-kelas?mata=${ujian.mataPelajaran}&ujianId=${ujian.id}`}>
      <div className='bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow cursor-pointer h-full overflow-hidden flex flex-col'>
        ...
        <div className='px-5 py-4 space-y-2 flex-grow'>
          ...
          <div className='flex justify-between text-sm'>
            ...
            <span className='font-medium text-green-600'>{ujian.selesai}</span>
          </div>
        </div>
      </div>
    </Link>
